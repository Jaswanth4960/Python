...
# def maindatabase():
conn = pyodbc.connect(driver="{sql server}", server="DESKTOP-UKA5IHV", database="jaswanth", trusted_connection="yes")
cursor = conn.cursor()
...
def Database():
    ...
    for data in fetch:
        clean_data = [str(item).strip("'") for item in data]
...
def SubmitData():
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or AGE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "":
        ...
        conn.commit()
        cursor.execute("select top 1 * from member ORDER BY mem_id DESC")
        fetch = cursor.fetchone()
        req_data = [str(item).strip("'") for item in fetch]
        tree.insert('','end',values=req_data)
        FIRSTNAME.set("")
        LASTNAME.set("")
        GENDER.set("")
        AGE.set("")
...
#         conn.commit()
#         cursor.execute("SELECT * FROM member where mem_id = ?",(mem_id))
#         fetch = cursor.fetchone()
#         updated_rw_data = [str(item).strip("'") for item in fetch]
#         tree.insert('', 'end', values=updated_rw_data)
#         FIRSTNAME.set("")
#         LASTNAME.set("")
#         GENDER.set("")
#         AGE.set("")
...
def UpdateData():
    if GENDER.get() == "":
        ...
        # Update the record in the Treeview widget
        updated_item = tree.selection()[0]
        tree.item(updated_item, values=(
            mem_id,
            FIRSTNAME.get(),
            LASTNAME.get(),
            GENDER.get(),
...
def count():
    ...
    messagebox.showinfo('Record Count', f'Total Records in MEMBER table: {count_result}')
...
def OnSelected(event):
    ...
    UpdateWindow.title("Update Contact List")
    width = 400
    height = 300
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
# btn_add.pack()
# btn_add_count = Button(MidLeft, text="COUNT", bg="#ffff66")
# btn_add_count.pack(side=LEFT, padx=5)
# btn_delete = Button(MidRight, text="DELETE", bg="red", command=DeleteData)
# btn_delete.pack(side=RIGHT)

btn_add = Button(MidLeft, text="+ ADD NEW", bg="#66ff66", command=AddNewWindow)
btn_add.grid(row=0, column=0, padx=(5, 10), pady=5)  # Increased padx to add extra space between buttons

btn_add_count = Button(MidLeft, text="COUNT", bg="#ff5722",command=count)
btn_add_count.grid(row=0, column=1, padx=(10, 5), pady=5, sticky="nsew")  # Increased padx to add extra space between buttons
